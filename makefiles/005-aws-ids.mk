...
#NAT Gateway
NAT_GW_A_ASSOCIATION_ID=$(shell aws ec2 describe-addresses --region $(REGION_A) --filters "Name=tag:Name,Values=$(NAT_GW_A_PUBLIC_IP_NAME)"   --output text --query "Addresses[0].AllocationId")
NAT_GW_B_ASSOCIATION_ID=$(shell aws ec2 describe-addresses --region $(REGION_B) --filters "Name=tag:Name,Values=$(NAT_GW_B_PUBLIC_IP_NAME)"   --output json --query "Addresses[0].AllocationId")
...
BASTION_A_SG=$(shell aws ec2 describe-instances  --region $(REGION_A) --instance-ids $(BASTION_A_ID) --query 'Reservations[].Instances[].SecurityGroups[].GroupId' --output text)
BASTION_B_SG=$(shell aws ec2 describe-instances  --region $(REGION_B) --instance-ids $(BASTION_B_ID) --query 'Reservations[].Instances[].SecurityGroups[].GroupId' --output text)
# Bastion IP
BASTION_A_PUBLIC_IP=$(shell aws ec2 describe-addresses --region $(REGION_A) --filters "Name=tag:Name,Values=$(BASTION_A_PUBLIC_IP_NAME)"   --output json --query "Addresses[0].PublicIp")
BASTION_B_PUBLIC_IP=$(shell aws ec2 describe-addresses --region $(REGION_B) --filters "Name=tag:Name,Values=$(BASTION_B_PUBLIC_IP_NAME)"   --output json --query "Addresses[0].PublicIp")
